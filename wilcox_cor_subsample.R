...
#WARNING!! ggplot boxplots are wrong when use facet.
# K_cor1=data.frame(cbind(as.numeric(unlist(K_cor)),rep(rep(list2,each=100),3),c(rep("PICRUSt",7),rep("PICRUSt2",7),rep("Tax4Fun",7))))
# colnames(K_cor1)=c("rho","Dataset","Method")
# K_cor1[,1]=as.numeric(as.character(K_cor1[,1]))
# K_cor1[,2]=factor(K_cor1[,2],levels=c("Human_KW","Human_TY","Gorilla","Mouse","Chicken","Soil_LWM","Soil_AAN"))
# 
# spear_group1=spear_group[,1:3]
# colnames(spear_group1)=c("rho","Method","Dataset")
# 
# pdf("wilcx/wilcx_spearman_subsample.pdf",height=15,width=10)
# plot1=ggplot(K_cor1, x = "Dataset", y = "rho", color = "blue", add = "jitter", show.legend=TRUE,
...
# plot2=plot1+geom_point(data=spear_group1,col="red",size=2)
# plot3=plot2+facet_wrap(~ Method, ncol=1)
# ggpar(plot3, ylim = c(-0.5,1))
# dev.off()
# 
# 
# plot1=ggplot(data = K_cor1, aes(x=Dataset, y=rho)) + geom_boxplot(col="blue")+geom_jitter(col="blue",size=2)+
#   theme(plot.title = element_text(size=5),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_rect(fill = 'white', colour = 'black'))+
#   labs(x = "Dataset", y = "Inference rho", size=6)+geom_point(data=spear_group1,col="red",size=2)+
#   coord_cartesian(y = c(-0.5,1))
# plot3=plot1+facet_wrap(~ Method, ncol=1)
# ggpar(plot3)
# 
# 
# 
# boxplot(K_cor, ylim = c(-0.5,1))
# stripchart(K_cor, vertical = TRUE, method = "jitter", add = TRUE, pch = 10,cex=0.5, col = 'blue')
# points(spear_group[,1][c(1:3,5,4,6:7,8:10,12,11,13:14,15:17,19,18,20,21)],col="red",cex=1,pch=16)
# abline(g)
